...
class TextEditor extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            caseTitle: '13 year old with a Type 1 Diabetes presenting high glucose level and heart rate.',
            parentItems: {}
        };
    }
    ...
    onChange({ state }) {
        ...
        if (WebViewBridge) {
            WebViewBridge.send(JSON.stringify(state));
        }
    }
        ...
        const ParentDragHandle = SortableHandle(() => {
            return (
                <span className="list-icon" contentEditable={false}>
                    ...
                    >
                        {this.state.parentItems[node.key] ? 'keyboard_arrow_right' : 'keyboard_arrow_down'}
                    </i>
        ...
        return (
            <div className="modal">
                ...
                <textarea
                    ...
                    onChange={(e) => { this.setState({ caseTitle: e.target.value }) }}
                    className="textarea"
                />
